...
public class Ayuda3 implements Screen {
    ...
    public void show() {
        ...
        imagenIzquierda = new Texture("Ayuda/Obstaculos.png");
    ...
    public void render(float delta) {
        ...
        String texto = Idiomas.t("ayuda3").replace("\\n", "\n");

        font.draw(batch,
            texto,
            0.3f,      // margen izquierdo
            2.8f,      // altura
            4.4f,      // ancho máximo (muy importante)
            Align.center,
            true);     // activa wrap automático
        ...
        if (mostrarBoton && Gdx.input.justTouched()) {
            ...
            if (btnSiguiente.getBoundingRectangle().contains(touch.x, touch.y)) {
                game.setScreen(new MenuPrincipal(game)); //Vuelta al menu
            }
        }
    }
